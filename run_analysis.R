...
if (!("reshape2" %in% rownames(installed.packages())) ) {
  ...
  activity_labels <- read.table("C:/Users/User/Downloads/UCI HAR Dataset/activity_labels.txt",col.names=c("activity_id","activity_name"))
  
  ## Read the dataframe's column names
  features <- read.table("C:/Users/User/Downloads/UCI HAR Dataset/features.txt")
  feature_names <-  features[,2]
  
  ## Read the test data and label the dataframe's columns
  testdata <- read.table("C:/Users/User/Downloads/UCI HAR Dataset/test/X_test.txt")
  colnames(testdata) <- feature_names
  
  ## Read the training data and label the dataframe's columns
  traindata <- read.table("C:/Users/User/Downloads/UCI HAR Dataset/train/X_train.txt")
  colnames(traindata) <- feature_names
  
  ## Read the ids of the test subjects and label the the dataframe's columns
  test_subject_id <- read.table("C:/Users/User/Downloads/UCI HAR Dataset/test/subject_test.txt")
  colnames(test_subject_id) <- "subject_id"
  
  ## Read the activity id's of the test data and label the the dataframe's columns
  test_activity_id <- read.table("C:/Users/User/Downloads/UCI HAR Dataset/test/y_test.txt")
  colnames(test_activity_id) <- "activity_id"
  
  ## Read the ids of the test subjects and label the the dataframe's columns
  train_subject_id <- read.table("C:/Users/User/Downloads/UCI HAR Dataset/train/subject_train.txt")
  colnames(train_subject_id) <- "subject_id"
  
  ## Read the activity id's of the training data and label 
  ##the dataframe's columns
  train_activity_id <- read.table("C:/Users/User/Downloads/UCI HAR Dataset/train/y_train.txt")
  ...
  write.table(mean_data,"C:/Users/User/Downloads/UCI HAR Dataset/tidy_movement_data.txt")
